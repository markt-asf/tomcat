...
public class TestRateLimitFilter extends TomcatBaseTest {
    ...
    private void testRateLimitWith4Clients(boolean exposeHeaders, boolean enforce) throws Exception {
        ...
        if (enforce) {
            ...
            Assert.assertEquals(HttpServletResponse.SC_TOO_MANY_REQUESTS, tc3.results[allowedRequests]);
            // subsequent requests dropped
            Assert.assertEquals(HttpServletResponse.SC_TOO_MANY_REQUESTS, tc4.results[allowedRequests]);
        }
        if (exposeHeaders) {
